...
class Install extends CI_Controller
{
  ...
  function validate_purchase_code()
  {
    ...
    $validation_response = $this->install_model->curl_request($purchase_code);
  ...
  function finalizing_setup($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'setup_admin') {
      $admin_data['name']       = sanitize($this->input->post('name'));
      $admin_data['email']      = sanitize($this->input->post('email'));
      ...
      $data['value']  = sanitize($this->input->post('system_name'));
      ...
      $data['value']  = sanitize($this->input->post('timezone'));
  ...
  function configure_routes()
  {
    ...
    $data_routes = str_replace('install',  'site',  $data_routes);
