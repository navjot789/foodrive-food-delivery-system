...
for ($i = 0; $i < 12; $i++) {
    ...
    $total_admin_revenue = $this->db->get('commission_details')->row()->admin_commission;
    $total_admin_revenue > 0 ? array_push($month_wise_income, $total_admin_revenue) : array_push($month_wise_income, 0);
}
// EACH DAY ORDER DATA
$each_day_orders = [];
for ($i = 1; $i <= date("t"); $i++) {
    $order_number = count($this->order_model->get_by_condition(["order_placed_at >=" => strtotime("$i " . date("M") . " " . date("Y") . "00:00:01"), "order_placed_at <=" => strtotime("$i " . date("M") . " " . date("Y") . "23:59:59")]));
    array_push($each_day_orders, $order_number);
}

$number_of_approved_restaurants = count($this->restaurant_model->get_all_approved());
$number_of_pending_restaurants = count($this->restaurant_model->get_all_pending());
?>
